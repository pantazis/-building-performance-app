...
5. If user-defined weightings are needed, user fills weighting settings.
6. User reviews the domain dashboard.
7. User fills smart-readiness information for each relevant domain.
8. System displays the final SRI score, class, and breakdowns.
...
| `SRI1` | Methodology Selection | `../view/SRI1-methodology-selection.html` | `I` | `SRI2` | Choose Method A or B, default/user-defined weightings, assessment date, and which SRI domains exist. |
| `SRI2` | User-defined Weightings Decision | `../view/SRI2-user-defined-weightings.html` | `SRI1` | `SRI3` if Yes, `SRI4` if No | Confirm whether custom weightings are required. Non-EU assessments should force user-defined weighting. |
| `SRI3` | Weighting Settings | `../view/SRI3-weighting-settings.html` | `SRI2` | `SRI4` | Fill impact weightings and domain weightings. Required totals must equal 100%. |
| `SRI4` | Domain Dashboard | `../view/SRI4-domain-dashboard.html` | `SRI2` or `SRI3` | `SRI5`-`SRI13`, then `SRI14` | Shows available SRI domains and their completion state based on domain presence and method. |
...
```text
Projects Dashboard → Building Tab → SRI: Methodology Selection → SRI: User-defined Weightings → SRI: Domain Dashboard → Domain Tab / Results
```

For domain pages, the current breadcrumb keeps a grouped label `SRI: Domain Tabs` before the specific domain tab, for example:

```text
Projects Dashboard → Building Tab → SRI: Methodology Selection → SRI: User-defined Weightings → SRI: Domain Dashboard → SRI: Domain Tabs → Heating Tab
```
...
| Data path | Meaning | UI requirement |
|---|---|---|
| `sri.hasDefaultWeightings` | Whether default weightings are used | If `false`, route to `SRI3`; if `true`, route to `SRI4`. |
...
| Non-EU weighting | If region is Non-EU, preferred weightings must be User-defined. |
